...
from foods.schema import product_schema_to_form_data
from foods.views import FoodCreateView  # adapte à ton module


@pytest.mark.django_db
class TestFoodCreateView:
    def setup_method(self):
        self.factory = RequestFactory()

    def test_get_form_without_barcode(self):
        """Le formulaire doit être vide si aucun code-barres n'est fourni."""
        request = self.factory.get("/foods/new/")
        view = FoodCreateView()
        view.request = request

        form = view.get_form()  # pyright: ignore[reportUnknownMemberType]

        assert isinstance(form, FoodForm), (
            "Le formulaire retourné n'est pas une instance de FoodForm"
        )
        assert form.initial == {}, (
            f"Le formulaire initial n'est pas vide : {form.initial}"
        )

    @patch("foods.views.fetch_product_data")
    def test_get_form_with_barcode(self, mock_fetch_product_data: MagicMock):
        ...
        request = self.factory.get("/foods/create/?barcode=123456")
        view = FoodCreateView()
        view.request = request
